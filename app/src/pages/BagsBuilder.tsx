...
import { bagOptions, calculatePrice, formatAED } from '../lib/data';
...
export default function BagsBuilder() {
  const { isRTL } = useLang();
  const [step, setStep] = useState(1);
  const [showCustomerModal, setShowCustomerModal] = useState(false);
  const [preview, setPreview] = useState({ bag: 0, leather: 0, size: 0, color: 0, box: 0 });
  ...
  const pricing = calculatePrice('bags', quantity, preview, []);
  ...
  const colorMap = ['#000000', '#5c3a1e', '#c4956a', '#1a2744', '#722f37', '#ff6b00'];
  const previewColor = colorMap[preview.color] || '#000000';
  const progress = (step / TOTAL_STEPS) * 100;

  const stepLabels = [
    isRTL ? 'العلامة' : 'Brand', isRTL ? 'الحقيبة' : 'Bag Type', isRTL ? 'الجلد' : 'Leather',
    isRTL ? 'الحجم' : 'Size', isRTL ? 'اللون' : 'Color', isRTL ? 'الصندوق' : 'Box', isRTL ? 'المراجعة' : 'Review'
  ];
    ...
    <div className="min-h-screen pt-[72px] pb-32" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="mb-10 p-6 rounded-2xl" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)' }}>
          ...
          <div className="flex items-center justify-center py-6">
            <svg width="140" height="140" viewBox="0 0 140 140">
              <rect x="20" y="30" width="100" height="85" rx="12" fill={previewColor} fillOpacity={0.3} stroke={previewColor} strokeWidth="2" />
              <path d="M40 30 C 40 5, 100 5, 100 30" fill="none" stroke={previewColor} strokeWidth="3" />
              <rect x="45" y="55" width="50" height="25" rx="4" fill="white" fillOpacity="0.9" />
              <text x="70" y="72" textAnchor="middle" fontSize="8" fontWeight="bold" fill="#1a1a2e">{brandName || (isRTL ? 'علامتك' : 'BRAND')}</text>
              <rect x="25" y="35" width="90" height="75" rx="8" fill="none" stroke={previewColor} strokeWidth="0.5" strokeOpacity="0.3" strokeDasharray="3,3" />
            </svg>
          </div>
          {pricing.total > 0 && (
        ...
        <AnimatePresence mode="wait">
          {step === 1 && <Step key="s1" title={isRTL ? 'اسم العلامة' : 'Brand Name'} desc={isRTL ? 'ما اسم علامتك التجارية؟' : 'What is your bag brand name?'}><input type="text" value={brandName} onChange={e => setBrandName(e.target.value)} placeholder={isRTL ? 'مثال: دار نايل' : 'e.g. Dar Nile'} className="w-full px-6 py-4 rounded-xl text-lg focus:border-[#d4af37] focus:outline-none" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }} /></Step>}
          {step === 2 && <Step key="s2" title={isRTL ? 'نوع الحقيبة' : 'Bag Type'} desc={isRTL ? 'اختر نوع الحقيبة' : 'Choose bag type'}><div className="grid grid-cols-2 md:grid-cols-3 gap-4">{L(bagOptions.bagType).map((opt, i) => <button key={opt} onClick={() => { setPreview(p => ({ ...p, bag: i })); nextStep(); }} className={`luxury-card p-5 text-center transition-all ${preview.bag === i ? 'border-[#d4af37]' : ''}`}><p className="font-medium" style={{ color: 'var(--text-primary)' }}>{opt}</p><p className="text-sm text-[#d4af37] mt-1">+{formatAED(bagOptions.bagType.prices[i])}</p></button>)}</div></Step>}
          {step === 3 && <Step key="s3" title={isRTL ? 'نوع الجلد' : 'Leather Type'} desc={isRTL ? 'اختر نوع الجلد' : 'Choose leather type'}><div className="grid grid-cols-2 md:grid-cols-3 gap-4">{L(bagOptions.leather).map((opt, i) => <button key={opt} onClick={() => { setPreview(p => ({ ...p, leather: i })); nextStep(); }} className={`luxury-card p-5 text-center transition-all ${preview.leather === i ? 'border-[#d4af37]' : ''}`}><p className="font-medium" style={{ color: 'var(--text-primary)' }}>{opt}</p><p className="text-sm text-[#d4af37] mt-1">+{formatAED(bagOptions.leather.prices[i])}</p></button>)}</div></Step>}
          {step === 4 && <Step key="s4" title={isRTL ? 'الحجم' : 'Size'} desc={isRTL ? 'اختر الحجم' : 'Choose size'}><div className="grid grid-cols-3 gap-4">{L(bagOptions.size).map((opt, i) => <button key={opt} onClick={() => { setPreview(p => ({ ...p, size: i })); nextStep(); }} className={`luxury-card p-6 text-center transition-all ${preview.size === i ? 'border-[#d4af37]' : ''}`}><p className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{opt}</p><p className="text-sm text-[#d4af37] mt-1">+{formatAED(bagOptions.size.prices[i])}</p></button>)}</div></Step>}
          {step === 5 && <Step key="s5" title={isRTL ? 'اللون' : 'Color'} desc={isRTL ? 'اختر لون الحقيبة' : 'Choose bag color'}><div className="grid grid-cols-3 md:grid-cols-6 gap-4">{L(bagOptions.color).map((opt, i) => <button key={opt} onClick={() => { setPreview(p => ({ ...p, color: i })); nextStep(); }} className={`luxury-card p-4 text-center transition-all ${preview.color === i ? 'border-[#d4af37] ring-2 ring-[#d4af37]/20' : ''}`}><div className="w-10 h-10 rounded-full mx-auto mb-2 border-2" style={{ backgroundColor: colorMap[i], borderColor: preview.color === i ? '#d4af37' : 'transparent' }} /><p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{opt}</p><p className="text-xs text-[#d4af37]">{bagOptions.color.prices[i] > 0 ? `+${formatAED(bagOptions.color.prices[i])}` : 'Free'}</p></button>)}</div></Step>}
          {step === 6 && (
            <Step key="s6" title={isRTL ? 'التفاصيل' : 'Details'} desc={isRTL ? 'الكمية والصور' : 'Quantity & Images'}>
              <div className="mb-6">
                <label className="text-sm font-medium mb-3 block" style={{ color: 'var(--text-secondary)' }}>{isRTL ? 'الكمية' : 'Quantity'}</label>
                <div className="flex items-center gap-4"><input type="range" min="100" max="5000" step="50" value={quantity} onChange={e => setQuantity(Number(e.target.value))} className="flex-1 accent-[#d4af37]" /><span className="text-2xl font-bold text-[#d4af37] w-24 text-right">{quantity.toLocaleString()}</span></div>
              </div>
          ...
          {step === 7 && (
            <Step key="s7" title={isRTL ? 'المراجعة' : 'Review'} desc={isRTL ? 'راجع طلبك' : 'Review your order'}>
              <div className="luxury-card p-8 space-y-3">
                {[{ label: isRTL ? 'العلامة' : 'Brand', value: brandName || '-' }, { label: isRTL ? 'النوع' : 'Type', value: L(bagOptions.bagType)[preview.bag] }, { label: isRTL ? 'الجلد' : 'Leather', value: L(bagOptions.leather)[preview.leather] }, { label: isRTL ? 'الحجم' : 'Size', value: L(bagOptions.size)[preview.size] }, { label: isRTL ? 'اللون' : 'Color', value: L(bagOptions.color)[preview.color] }, { label: isRTL ? 'الصندوق' : 'Box', value: L(bagOptions.boxType)[preview.box] }, { label: isRTL ? 'الكمية' : 'Quantity', value: quantity.toLocaleString() }, { label: isRTL ? 'المجموع' : 'Total', value: formatAED(pricing.total), gold: true }].map((item, i) => (
      ...
      <CustomerDetailsModal
        ...
        orderData={{
          ...
          categoryAr: 'حقائب',
          productSelections: {
            bagType: bagOptions.bagType.en[preview.bag],
            leather: bagOptions.leather.en[preview.leather],
            size: bagOptions.size.en[preview.size],
            color: bagOptions.color.en[preview.color],
          },
          packagingSelections: {
            box: bagOptions.boxType.en[preview.box],
          },
          quantity,
